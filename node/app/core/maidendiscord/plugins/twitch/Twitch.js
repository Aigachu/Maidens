...
class Twitch {
    ...
    ttvannAddStream(guild_id, stream_user) {
        if (this.guilds[guild_id].ttvann.streams.indexOf(stream_user) > -1) {
            return false;
        }
    ...
    ttvannAddStreamLive(guild_id, stream_user) {
        if (this.guilds[guild_id].ttvann.live.indexOf(stream_user) > -1) {
            return false;
        }
    ...
    ttvannRemoveStream(guild_id, stream_user) {
        let streams_index = this.guilds[guild_id].ttvann.streams.indexOf(stream_user);
        let live_index = this.guilds[guild_id].ttvann.live.indexOf(stream_user);

        if (streams_index > -1)
            this.guilds[guild_id].ttvann.streams.splice(streams_index, 1);

        if (live_index > -1)
            this.guilds[guild_id].ttvann.live.splice(live_index, 1);
