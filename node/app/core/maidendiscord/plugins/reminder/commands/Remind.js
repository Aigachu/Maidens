...
class Remind extends Command {
  ...
  tasks(data) {
    ...
    // Parse the input and find out what to do.
		let parsed_data = this.parse(data.msg, data.input.raw);
   ...
   */
    ...
    if (receiver.indexOf('@!') >= 0) {
      let member = message.guild.members.find('id', receiver.replace('@!', ''));
      return {object_type: 'user', object_id: member.id};
    }

    // If the tag is a role tag, get the role.
    if (receiver.indexOf('@&') >= 0) {
      let role = message.guild.roles.find('id', receiver.replace('@&', ''));
    ...
    if (receiver.indexOf('#') >= 0) {
      let channel = message.guild.channels.find('id', receiver.replace('#', ''));
